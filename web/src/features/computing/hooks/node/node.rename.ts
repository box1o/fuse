...
import type { ComputeNode, RenameNodeRequest } from "../../types/node.types";
...
export const useRenameNode = () => {
    ...
    const mutation = useMutation<ComputeNode, Error, RenameNodeVariables>({
        mutationKey: [COMPUTE_QUERY_KEYS.RENAME],
        mutationFn: async ({ nodeId, request }) => {
            if (!nodeId) throw new Error("Missing compute node ID");

            const response = await computeService.rename(nodeId, request);

            if (!response.success || !response.data) {
                throw new Error(response.error || "Failed to rename compute node");
            }
        ...
        onSuccess: (data) => {
            ...
            toast.success("Compute node renamed");
        },
        onError: (error) => {
            setError(error.message);
            toast.error(error.message || "Failed to rename compute node");
        },
    });
